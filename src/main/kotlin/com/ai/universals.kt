...
internal class CalculationFailure(msg: String): Exception(msg)
internal class NonexistentValueException(msg: String): Exception(msg)
open class Encryptor<in XType, YType>(ex: ((XType) -> Number)?, ey: ((YType) -> Number)?, d: ((Number) -> YType)?) {
...
operator fun <T> Collection<T>.get(index: Int) = elementAt(index)